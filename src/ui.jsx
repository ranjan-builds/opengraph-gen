...
export const Toggle = ({ label, checked, onChange }) => (
  <div className="flex items-center justify-between py-2">
    ...
    >
      <div
        className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-transform duration-200 ${
...
export const CodeBlock = ({ label, code }) => (
  <div className="bg-slate-900 rounded-xl border border-slate-800 text-sm overflow-hidden mb-4">
    <div className="flex items-center justify-between px-3 py-2 bg-slate-800/50 border-b border-slate-800">
      <h3 className="text-slate-300 font-medium text-xs uppercase tracking-wider">
        {label}
      </h3>
      <button
        onClick={() => {
    ...
    <div className="p-3 overflow-x-auto custom-scrollbar">
      <code className="font-mono text-xs text-blue-300 whitespace-pre">
        {code}
      </code>
    </div>
  </div>
