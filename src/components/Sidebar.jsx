...
import { ColorPicker, Toggle, Range, CodeBlock } from "./Controls";
import {
  FONTS,
  GRADIENT_PRESETS,
  PATTERNS,
...
const Sidebar = ({
  ...
  // Safe domain extraction for Twitter meta
  const getDomain = (url) => {
    try {
      if (!url) return "";
      const u = new URL(url);
  ...
  return (
    <aside className="w-full lg:w-80 h-1/2 lg:h-full bg-[#1D1D1D] border-t lg:border-t-0 lg:border-r border-[#333] flex flex-col z-20 order-2 lg:order-1 shrink-0">
      {/* Tabs */}
      <div className="flex p-2 gap-1 border-b border-[#333] sticky top-0 bg-[#1D1D1D] z-10 shrink-0">
        {[
          ...
          <button
            ...
            className={`flex-1 flex flex-col items-center gap-1 py-2 rounded-md text-xs font-medium transition-all ${
              activeTab === tab.id
                ? "bg-[#333] text-blue-400 shadow-sm"
      ...
      <div className="p-5 space-y-8 pb-20 overflow-y-auto custom-scrollbar flex-1">
                ...
                <button
                  ...
                  className={`flex items-center gap-3 p-3 rounded-xl border transition-all text-left group ${
                    config.template === t.id
                      ...
                      : "bg-[#0A0A0A] border-transparent hover:border-[#333] text-slate-400"
                  }`}
                >
                  <div
        ...
        {activeTab === "content" && (
          <div className="space-y-6">
            {/* URL Fetcher */}
            <div className="space-y-2 bg-[#0A0A0A] p-3 rounded-xl border border-[#333]">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="https://..."
                  className="flex-1 bg-[#1D1D1D] border-0 rounded text-xs px-2 focus:ring-1 focus:ring-blue-500"
                />
                <button
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full bg-[#0A0A0A] border border-[#333] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all placeholder-slate-600"
              ...
              <textarea
                ...
                className="w-full bg-[#0A0A0A] border border-[#333] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all placeholder-slate-600 resize-none"
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  className="bg-[#0A0A0A] border border-[#333] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center gap-3 p-3 bg-[#0A0A0A] border border-[#333] rounded-lg cursor-pointer hover:border-slate-500 transition-colors group">
                <div className="w-10 h-10 rounded bg-[#1D1D1D] flex items-center justify-center group-hover:bg-[#333] overflow-hidden relative">
                  {config.image ? (
                    <img
                      src={config.image}
        ...
        {activeTab === "design" && (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex rounded-lg bg-[#0A0A0A] p-1 border border-[#333]">
                <button
              ...
              {config.bgType === "gradient" ? (
                <div className="space-y-3 p-3 bg-[#0A0A0A] rounded-xl border border-[#333]">
                  <div className="grid grid-cols-5 gap-2 max-h-48 overflow-y-auto custom-scrollbar p-1">
                    {GRADIENT_PRESETS.map((g, i) => (
                      <button
                        ...
                        style={{
                          background: `linear-gradient(${g.dir}, ${g.start}, ${g.end})`,
                        }}
                        title={g.name}
                      >
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    <ColorPicker
                      ...
                      onChange={(v) =>
                        setConfig({ ...config, gradientStart: v })
                      }
                    />
                    <ColorPicker
                      label="End"
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setConfig({ ...config, gradientDir: e.target.value })
                      }
                      className="w-full bg-[#1D1D1D] text-xs text-white p-2 rounded border border-[#333] outline-none"
                    >
            ...
            <div className="space-y-3 border-t border-[#333] pt-6">
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 text-xs border rounded-lg transition-all ${
                      config.pattern === p.value
                        ...
                        : "border-[#333] text-slate-400 hover:border-slate-600"
                    }`}
                  >
                    {p.name}
                  </button>
              ...
              {config.pattern !== "none" && (
                <div className="p-3 bg-[#0A0A0A] rounded-xl border border-[#333] space-y-3">
                  <ColorPicker
                    label="Grid Color"
                  ...
                  <div className="space-y-2 pt-2 border-t border-[#333]">
                    ...
                    <div className="grid grid-cols-3 gap-1 mb-2">
                      {FADE_OPTIONS.map((opt) => (
                        <button
                          key={opt.id}
                          onClick={() =>
                            setConfig({ ...config, gridFade: opt.id })
                          }
                          className={`flex items-center justify-center p-2 rounded border text-[11px] transition-all ${
                            config.gridFade === opt.id
                              ? "bg-blue-500/20 border-blue-500 text-blue-400"
                              : "bg-[#1D1D1D] border-[#333] text-slate-400 hover:border-slate-600"
                          }`}
                        >
                          {opt.label}
                        </button>
                    ...
                    {config.gridFade !== "none" && (
                      <Range
                        label="Fade Offset"
                        value={config.gridFadeOffset}
                        min={0}
                        max={100}
                        step={5}
                        unit="%"
                        onChange={(v) =>
                          setConfig({ ...config, gridFadeOffset: v })
                        }
                      />
                    )}
                  </div>
              ...
              {/* Noise */}
              <div className="p-3 bg-[#0A0A0A] rounded-xl border border-[#333] space-y-3 mt-3">
                <Range
                  label="Noise"
                  value={config.noise}
                  min={0}
                  max={0.5}
                  step={0.05}
                  onChange={(v) => setConfig({ ...config, noise: v })}
                />
              </div>
            </div>
            ...
            <div className="space-y-3 border-t border-[#333] pt-6">
              ...
              <div className="relative mb-3">
                <select
                  ...
                  className="w-full appearance-none bg-[#0A0A0A] border border-[#333] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                >
                  {FONTS.map((f) => (
          ...
          <div className="space-y-6">
            ...
            {hostedUrl ? (
              <div className="bg-emerald-900/20 border border-emerald-500/20 p-4 rounded-xl flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-emerald-500/60 text-xs mb-3">
                    Your image is live and ready for SEO tags.
                  </p>
                  <div className="flex gap-2">
                    <input
                      type="text"
              ...
              <div className="bg-blue-900/10 border border-blue-500/10 p-4 rounded-xl flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs">
                    Click the cloud icon in the header to generate a public
                    link.
                  </p>
                </div>
              </div>
            ...
            <div>
              ...
              <div className="p-3 bg-slate-900/50 rounded-lg text-xs text-slate-400 border border-slate-800">
                LinkedIn uses the same Open Graph (<code>og:*</code>) tags as
                Facebook. Use a 1200×627px image for best results.
              </div>
            </div>
          </div>
        )}
